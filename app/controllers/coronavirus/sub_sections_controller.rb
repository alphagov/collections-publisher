  ...
  class SubSectionsController < ApplicationController
    ...
    def create
      ...
      if @sub_section.save && draft_updater.send
        redirect_to coronavirus_page_path(page.slug), notice: helpers.t("coronavirus.new_sub_section.success")
      else
    ...
    def update
      ...
      if @sub_section.update(sub_section_params) && draft_updater.send
        redirect_to coronavirus_page_path(page.slug), notice: helpers.t("coronavirus.edit_sub_section.success")
      else
        @sub_section.errors.add :base, draft_updater.errors.to_sentence
        render :edit
      end
    ...
    def destroy
      sub_section = page.sub_sections.find(params[:id])
      message = { notice: helpers.t("coronavirus.summary.sub_sections.delete.success") }

      SubSection.transaction do
        sub_section.destroy!

        unless draft_updater.send
          message = { alert: helpers.t("coronavirus.summary.sub_sections.delete.failed") }

module Coronavirus
  class ReorderSubSectionsController < ApplicationController
    ...
    def update
      ...
      if success
        message = helpers.t("coronavirus.reorder_sub_sections.success")
        redirect_to coronavirus_page_path(page.slug), notice: message
      else
        message = helpers.t("coronavirus.reorder_sub_sections.failed", error: draft_updater.errors.to_sentence)
